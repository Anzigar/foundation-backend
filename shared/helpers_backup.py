...
def detect_uuid_columns(query: str) -> list:
    ...
    # For INSERT statements, parse the actual column names
    if 'insert into' in query_lower and 'values' in query_lower:
        try:
            # Extract column names from INSERT statement
            # Pattern: INSERT INTO table (col1, col2, ...) VALUES (...)
            import re
            columns_match = re.search(r'\((.*?)\)\s*values', query_lower)
            if columns_match:
                columns_text = columns_match.group(1)
                # Split by comma and clean up column names
                columns = [col.strip() for col in columns_text.split(',')]
                
                # Check each column position for UUID patterns
                uuid_column_patterns = ['id', 'project_id', 'user_id', 'article_id', 'event_id', 'blog_id']
                for i, column in enumerate(columns):
                    if any(pattern in column for pattern in uuid_column_patterns):
                        uuid_positions.append(i)
        except Exception:
            # If parsing fails, fall back to empty list (no UUID casting)
            pass
